...
int FFGLShader::Compile(const std::string& vtxProgram, const std::string& fragProgram, const std::map<std::string, int>& attributeValues)
{
	return Compile(vtxProgram.c_str(), fragProgram.c_str(), &attributeValues);
}
...
GLint FFGLShader::FindAttribute(const GLchar * name)
{
...
{
	return FFGLShader::Compile(vtxProgram, fragProgram, nullptr);
}

int FFGLShader::Compile(const char *vtxProgram, const char *fragProgram, const std::map<std::string, int>* attributeValues)
{
	if (m_glProgram == 0)
		CreateGLResources();
	...
	if (doLink)
	{
		// Set attribute values before program linkage stage
		if (attributeValues != nullptr)
		{
			for (auto attributeValue : *attributeValues)
			{
				auto attrName{ attributeValue.first };
				auto value{ attributeValue.second };
				glBindAttribLocation(m_glProgram, value, attrName.c_str());
			}
		}
